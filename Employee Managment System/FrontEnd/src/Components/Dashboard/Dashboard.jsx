...
export default function Dashboard() {
  ...
  const deleteUser = async (id, role) => {
    try {
      ...
      const updatedData = combinedData.filter(item => item._id !== id);
  ...
  const updateSalary = async (id, role) => {
    try {
      ...
      const updatedData = combinedData.map(item => item._id === id ? { ...item, salary: newSalary } : item);
      setCombinedData(updatedData);
      ...
      <div className="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-purple-600 text-white p-6 rounded-lg shadow-md">
          ...
          <p className="text-4xl mt-4">
            ...
            {userRole === 'manager' && project.managerProjectsLength}
