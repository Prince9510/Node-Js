...
export default function Team() {
  ...
  const updateSalary = async (id, role) => {
    try {
      ...
      const updatedData = role === 'manager' ? managers.map(item => item.id === id ? { ...item, salary: newSalary } : item) : employees.map(item => item.id === id ? { ...item, salary: newSalary } : item);
  ...
  const deleteUser = async (id, role) => {
    try {
      ...
      const updatedData = role === 'manager' ? managers.filter(item => item.id !== id) : employees.filter(item => item.id !== id);
      role === 'manager' ? setManagers(updatedData) : setEmployees(updatedData);
    } catch (error) {
