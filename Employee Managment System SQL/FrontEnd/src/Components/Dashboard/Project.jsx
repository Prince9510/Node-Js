...
export default function Project() {
  ...
  const fetchProjects = async () => {
    try {
      ...
      const { adminProjects, assigneToOwnEmployee, assigneMeProjects, employeeProjects } = response.data;
      setProjects(adminProjects || []);
      setAssigneToOwnEmployee(assigneToOwnEmployee || []);
      setAssigneMeProjects(assigneMeProjects || []);
  ...
  const filteredProjects = (projects) => {
    return projects.filter(project => {
      return (filterStatus === 'All' || project.status === filterStatus) &&
             (filterAssignedTo === 'All' || project.assignetoemployee === filterAssignedTo || project.assigneto === filterAssignedTo) &&
  ...
  const handleDeleteClick = async (projectId) => {
    ...
    try {
      ...
      setAssigneToOwnEmployee(assigneToOwnEmployee.filter(project => project.id !== projectId));
      setAssigneMeProjects(assigneMeProjects.filter(project => project.id !== projectId));
  ...
  const handleStatusUpdateClick = async (project) => {
    const token = localStorage.getItem('token');
    try {
        const response = await axios.put(`${serverUrl}/project/statusUpdate?id=${project.id}`, {}, {
            headers: {
                Authorization: `Bearer ${token}`,
            },
        });
        ...
        if (updatedProject && updatedProject.id) {
            ...
            setAssigneToOwnEmployee(prevProjects => prevProjects.map(p => p.id === updatedProject.id ? updatedProject : p));
            setAssigneMeProjects(prevProjects => prevProjects.map(p => p.id === updatedProject.id ? updatedProject : p));
        }
    ...
    <>
...

        {
          userRole === 'admin' ? (
            <select value={filterAssignedTo} onChange={(e) => setFilterAssignedTo(e.target.value)} className="py-2 px-4 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
              ...
              {[...new Set(projects.map(project => project.assigneto))].map((name, i) => (
            ...
            <select value={filterAssignedTo} onChange={(e) => setFilterAssignedTo(e.target.value)} className="py-2 px-4 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
              <option value="All">All Assignees</option>
              {[...new Set(assigneToOwnEmployee.map(project => project.assigneto))].map((name, i) => (
                <option key={i} value={name}>{name}</option>
              ))}
            </select>
          )
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4">
            {filteredProjects(projects).map((project, index) => (
              <div key={index} className={`p-4 rounded-lg shadow-md border ${getPriorityColor(project.priority)}`}>
                ...
                <p className="mb-1 text-sm"><strong>Assigned To:</strong> {project.assigneto}</p>
                <p className="mb-1 text-sm"><strong>Status:</strong> {project.status}</p>
                <p className="mb-1 text-sm"><strong>Start Date:</strong> {filterDate(project.startdate)}</p>
                <p className="mb-1 text-sm"><strong>Due Date:</strong> {filterDate(project.enddate)}</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4"> 
            {filteredProjects(assigneMeProjects).map((project, index) => (
              <div key={index} className={`p-4 rounded-lg shadow-md border ${getPriorityColor(project.priority)}`}>
                ...
                <p className="mb-1 text-sm"><strong>Description:</strong> {project.projectdescription}</p>
                <p className="mb-1 text-sm"><strong>Priority:</strong> {project.priority}</p>
                <p className="mb-1 text-sm"><strong>Assigned By:</strong> {project.assigneby}</p>
                <p className="mb-1 text-sm"><strong>Status:</strong> {project.status}</p>
                <p className="mb-1 text-sm"><strong>Start Date:</strong> {filterDate(project.startdate)}</p>
                <p className="mb-1 text-sm"><strong>Due Date:</strong> {filterDate(project.enddate)}</p>
                <div className="flex space-x-2 mt-2">
...

      {
        ...
        <>
          <h2 className="text-xl font-bold mb-4 mt-8">Assigned to Me</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4"> 
            {filteredProjects(employeeProjects).map((project, index) => (
              <div key={index} className={`p-4 rounded-lg shadow-md border ${getPriorityColor(project.priority)}`}>
                <h2 className="text-lg font-bold mb-1">{project.projectname}</h2>
                <p className="mb-1 text-sm"><strong>Description:</strong> {project.projectdescription}</p>
                <p className="mb-1 text-sm"><strong>Priority:</strong> {project.priority}</p>
