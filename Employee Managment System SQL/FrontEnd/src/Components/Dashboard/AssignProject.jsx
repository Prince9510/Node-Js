...
export default function AssignProject() {
  ...
  useEffect(() => {
    if (project) {
      setProjectName(project.projectname); // Corrected to match the property name
      setProjectDescription(project.projectdescription); // Corrected to match the property name
      setPriority(project.priority);
      setStatus(project.status);
      setStartDate(project.startdate.split('T')[0]); // Format date to YYYY-MM-DD
      setEndDate(project.enddate.split('T')[0]); // Format date to YYYY-MM-DD
      setAssigneeTo(project.assigneto?.id || '');
    }
  }, [project]);
  ...
  const handleProjectSubmit = async (e) => {
    ...
    if(userRole === 'admin'){
      const selectedManagerObj = managers.find(manager => manager.id = assigneeTo);
    ...
    try {
      if (project) {
        await axios.put(`http://localhost:5000/project/updateProject?id=${project.id}`, projectData, {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
        // toast.success('Project updated successfully');
        localStorage.setItem('message' , 'Project updated successfully')
        navigate('/project')
      } else {
    ...
    const selectedEmployeeObj = employee.find(employee => employee.id = assigneeTo); 

    

    // console.log('selectedEmployeeObj' + selectedEmployeeObj.assigneeTo)
    if (!selectedEmployeeObj) {
      toast.error('Invalid Employee ID FRONTEND');
      return;
    }
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // console.log('Employee Response:', responseEmployees.data.employeeData); 
